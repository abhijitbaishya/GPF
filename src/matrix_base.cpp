...
matrix_row	matrix_row::operator=(matrix_row& row)
{
	...
	this->~matrix_row();			//destroy the current object
	if(row.num_elements != 0)
	try
	{
		this->row_ptr = new int[row.num_elements];	//allocate same number of elements as row
...
void matrix_row::insert(int pos,int val)
{
	...
	{
		try{
			this->_capacity = (												//recalcute the _capacity
							num_elements % 16 ? 
							num_elements - (num_elements % 16) + 16 : 
							num_elements
						 );
	...
	for(int i = pos ; i<num_elements-1; i++)
		this->row_ptr[i+1] = this->row_ptr[i];	//shift the entire array to the right
		
	this->row_ptr[pos] = val;	//assign the value
}
